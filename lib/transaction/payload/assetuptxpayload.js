...
function AssetUpTxPayload(options) {
  ...
  this.externalPayoutScript = Buffer.alloc(PUBKEY_ID_SIZE).toString("hex");
  ...
  if (options) {
    ...
    if(options.collateralAddress != null) {
      this.collateralAddress = options.collateralAddress; // Script.fromAddress(options.payoutAddress).toHex();
    }
...
AssetUpTxPayload.fromBuffer = function fromBuffer(rawPayload) {
    ...
    payload.ownerAddress = payloadBufferReader
      .read(PUBKEY_ID_SIZE)
      .toString('hex');
    var collateralAddressSize = payloadBufferReader.readVarintNum();
    payload.collateralAddress = payloadBufferReader.read(collateralAddressSize).toString('hex');
...
AssetUpTxPayload.prototype.validate = function () {
  ...
  AssetUpTxPayload.prototype.toBuffer = function toBuffer(options) {
    ...
    payloadBufferWriter.write(Buffer.from(this.targetAddress, 'hex').reverse());
    ...
    payloadBufferWriter.write(Buffer.from(this.ownerAddress, 'hex').reverse());
    var collateralAddress = Buffer.from(this.collateralAddress, 'hex');
    payloadBufferWriter.writeVarintNum(collateralAddress.length);
    payloadBufferWriter.write(collateralAddress);
