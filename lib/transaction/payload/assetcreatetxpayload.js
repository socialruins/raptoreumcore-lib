...
AssetCreateTxPayload.fromBuffer = function fromBuffer(rawPayload) {
    ...
    if(!payload.isRoot) {
      var rootId = payloadBufferReader.readVarintNum();
...
AssetCreateTxPayload.prototype.validate = function () {
  ...
  AssetCreateTxPayload.prototype.toJSON = function toJSON(options) {
    ...
    var payloadJSON = {
        ...
        externalPayoutAddress: this.externalPayoutAddress === NULL_ADDRESS 
          ? "N/A" 
          : new Script(this.collateralAddress)
            .toAddress(network)
            .toString(),
  ...
  AssetCreateTxPayload.prototype.toBuffer = function toBuffer(options) {
    ...
    if(!this.isRoot) {
      var rootId = Buffer.from(this.rootId, 'utf8');
      payloadBufferWriter.writeVarintNum(rootId.length);
      payloadBufferWriter.write(rootId);
